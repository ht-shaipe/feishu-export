...
impl ExportLog {
    pub fn new(output_dir: &Path, space_id: &str) -> std::io::Result<Self> {
        ...
        writeln!(
            file,
            "# feishu-export log — space_id={} started={}",
            space_id,
            Utc::now().to_rfc3339()
        )?;
        drop(file);
    ...
    pub fn append_success(
        ...
        let entry =
            ExportLogEntry::success(title.to_string(), obj_token.to_string(), obj_type.to_string(), local_path.display().to_string());
        let mut file = std::fs::OpenOptions::new()
            .create(true)
            .append(true)
            .open(&self.path)?;
    ...
    pub fn append_failed(
        ...
        let entry =
            ExportLogEntry::failed(title.to_string(), obj_token.to_string(), obj_type.to_string(), error.to_string());
        let mut file = std::fs::OpenOptions::new()
            .create(true)
            .append(true)
            .open(&self.path)?;
